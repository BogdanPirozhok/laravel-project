<template>
    <div class="footer__top">
        <div class="footer__info">
            <a
                href="/"
                class="logo"
            >
                <img
                    src="/library/public/img/logo-white.svg"
                    alt=""
                >
            </a>
            <span class="footer__text">Производство рыбных продуктов</span>
            <span class="footer__text">г. Сосновоборск, ул. Заводская, 1 корпус 40</span>
            <a
                href="tel:88006005963"
                class="footer__text"
            >8 (800) 600-59-63 (служба качества)</a>
            <a
                href="mailto:info@delsy.ru "
                class="footer__text"
            >info@delsy.ru </a>

            <a
                href="#"
                class="footer__link"
            >Юридическая информация и публичная оферта</a>

            <div class="soc-seti white">
                <a
                    href="#"
                    class="soc-seti__link"
                    target="_blank"
                >
                    <svg
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                    >
                        <path
                            d="M13.8584 24V13.0533H17.5313L18.0824 8.78588H13.8584V6.06176C13.8584 4.82664 14.2 3.98492 15.9732 3.98492L18.231 3.98399V0.167076C17.8406 0.116334 16.5002 0 14.9403 0C11.6827 0 9.45258 1.98836 9.45258 5.63912V8.78588H5.76855V13.0533H9.45258V24H13.8584Z"
                        />
                    </svg>
                </a>
                <a
                    href="#"
                    class="soc-seti__link"
                    target="_blank"
                >
                    <svg
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                    >
                        <path
                            d="M12.2367 24C12.1572 24 12.0777 24 11.9977 23.9996C10.1164 24.0042 8.37813 23.9564 6.68763 23.8535C5.13777 23.7592 3.72304 23.2236 2.59617 22.3048C1.50885 21.4182 0.766326 20.2194 0.389297 18.7421C0.0611596 17.456 0.0437639 16.1935 0.0271006 14.9723C0.0150152 14.0962 0.00256357 13.058 0 12.0022C0.00256357 10.942 0.0150152 9.90377 0.0271006 9.02761C0.0437639 7.80666 0.0611596 6.54414 0.389297 5.25782C0.766326 3.78052 1.50885 2.58173 2.59617 1.69513C3.72304 0.776302 5.13777 0.240717 6.68782 0.146417C8.37831 0.043695 10.117 -0.00427877 12.0023 0.000298881C13.8841 -0.00372945 15.6219 0.043695 17.3124 0.146417C18.8622 0.240717 20.277 0.776302 21.4038 1.69513C22.4913 2.58173 23.2337 3.78052 23.6107 5.25782C23.9388 6.54396 23.9562 7.80666 23.9729 9.02761C23.985 9.90377 23.9976 10.942 24 11.9978V12.0022C23.9976 13.058 23.985 14.0962 23.9729 14.9723C23.9562 16.1933 23.939 17.4558 23.6107 18.7421C23.2337 20.2194 22.4913 21.4182 21.4038 22.3048C20.277 23.2236 18.8622 23.7592 17.3124 23.8535C15.6935 23.952 14.0304 24 12.2367 24V24ZM11.9977 22.1246C13.8484 22.129 15.5477 22.0823 17.1985 21.982C18.3704 21.9108 19.3865 21.5304 20.2189 20.8517C20.9883 20.2242 21.5183 19.3584 21.7939 18.2785C22.0671 17.2079 22.0828 16.0583 22.098 14.9467C22.1099 14.0764 22.1224 13.0455 22.1249 12C22.1224 10.9542 22.1099 9.92354 22.098 9.05324C22.0828 7.94161 22.0671 6.79207 21.7939 5.72126C21.5183 4.6413 20.9883 3.77558 20.2189 3.14807C19.3865 2.46948 18.3704 2.08917 17.1985 2.01794C15.5477 1.91742 13.8484 1.87109 12.0021 1.87512C10.1518 1.87073 8.45229 1.91742 6.80153 2.01794C5.62961 2.08917 4.61352 2.46948 3.78109 3.14807C3.01165 3.77558 2.48172 4.6413 2.20614 5.72126C1.93294 6.79207 1.91719 7.94142 1.90199 9.05324C1.89009 9.92428 1.87764 10.9557 1.87507 12.0022C1.87764 13.044 1.89009 14.0757 1.90199 14.9467C1.91719 16.0583 1.93294 17.2079 2.20614 18.2785C2.48172 19.3584 3.01165 20.2242 3.78109 20.8517C4.61352 21.5303 5.62961 21.9106 6.80153 21.9818C8.45229 22.0823 10.1521 22.1292 11.9977 22.1246V22.1246ZM11.953 17.8594C8.7222 17.8594 6.09343 15.2309 6.09343 12C6.09343 8.76906 8.7222 6.14057 11.953 6.14057C15.1841 6.14057 17.8126 8.76906 17.8126 12C17.8126 15.2309 15.1841 17.8594 11.953 17.8594V17.8594ZM11.953 8.01558C9.75605 8.01558 7.9685 9.80306 7.9685 12C7.9685 14.1969 9.75605 15.9844 11.953 15.9844C14.1502 15.9844 15.9376 14.1969 15.9376 12C15.9376 9.80306 14.1502 8.01558 11.953 8.01558V8.01558ZM18.4689 4.26557C17.6923 4.26557 17.0626 4.89509 17.0626 5.67182C17.0626 6.44856 17.6923 7.07808 18.4689 7.07808C19.2457 7.07808 19.8752 6.44856 19.8752 5.67182C19.8752 4.89509 19.2457 4.26557 18.4689 4.26557Z"
                        />
                    </svg>
                </a>
            </div>
        </div>
        <div class="footer__column">
            <span
                :class="{active: active.includes('partners')}"
                class="footer__title"
                @click="toggle('partners')"
            >Партнерам</span>
            <slide-up-down :active="active.includes('partners')">
                <ul class="footer__list">
                    <li class="footer__item">
                        <a
                            href="/partnership"
                            class="footer__link"
                        >Сотрудничество</a>
                    </li>
                    <li class="footer__item">
                        <a
                            href="/purchases"
                            class="footer__link"
                        >Закупки</a>
                    </li>
                    <!--<li class="footer__item">
                        <a href="#" class="footer__link">Реализация неликвидов</a>
                    </li>-->
                </ul>
            </slide-up-down>
        </div>

        <div class="footer__column">
            <span
                :class="{active: active.includes('links')}"
                class="footer__title"
                @click="toggle('links')"
            >Ссылки</span>
            <slide-up-down :active="active.includes('links')">
                <ul class="footer__list">
                    <li class="footer__item">
                        <a
                            href="/work"
                            class="footer__link"
                        >Работа</a>
                    </li>
                    <li class="footer__item">
                        <a
                            href="/catalog"
                            class="footer__link"
                        >Каталог</a>
                    </li>
                    <li class="footer__item">
                        <a
                            href="/news"
                            class="footer__link"
                        >Новости</a>
                    </li>
                    <li class="footer__item">
                        <a
                            href="/recipes"
                            class="footer__link"
                        >Рецепты</a>
                    </li>
                </ul>
            </slide-up-down>
        </div>

        <div class="footer__column">
            <span
                :class="{active: active.includes('company')}"
                class="footer__title"
                @click="toggle('company')"
            >Компания</span>
            <slide-up-down :active="active.includes('company')">
                <ul class="footer__list">
                    <li class="footer__item">
                        <a
                            href="/about"
                            class="footer__link"
                        >О нас</a>
                    </li>
                    <li class="footer__item">
                        <a
                            href="/quality"
                            class="footer__link"
                        >Контроль качества</a>
                    </li>
                    <li class="footer__item">
                        <a
                            href="/networks"
                            class="footer__link"
                        >Сети и дистрибьюторы</a>
                    </li>
                    <li class="footer__item">
                        <a
                            href="/contacts"
                            class="footer__link"
                        >Контакты</a>
                    </li>
                </ul>
            </slide-up-down>
            <!--            <a-->
            <!--                href="#"-->
            <!--                class="btn-border violet"-->
            <!--            >-->
            <!--                Центр Обратной связи-->
            <!--            </a>-->
        </div>
    </div>
</template>

<script>
export default {
	name: "FooterTop",
	data() {
		return {
			active: [],
			desktop: true
		}
	},
	mounted() {
		this.ifIsMobile();
		window.addEventListener('resize', this.ifIsMobile)
	},
	methods: {
		toggle(value) {
			if(!this.desktop){
				if (this.active.includes(value)) {
					this.active.splice(this.active.indexOf(value), 1);
				} else {
					this.active.push(value)
				}
			}
		},
		ifIsMobile(){
			if(window.innerWidth >= 768){
				this.desktop = true;
				this.active.push('company', 'partners', 'links')
			} else {
				this.active = [];
				this.desktop = false;
			}
		}
	}
}
</script>

<style scoped>

</style>
